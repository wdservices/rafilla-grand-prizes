...
const CATEGORIES = ["All categories", "Auto", "Tech", "Property", "Jewelry", "Home", "Experience"] as const;
...
export function AdminPartnersPage() {
  ...
  const submitInvite = () => {
    if (!inviteForm.company || !inviteForm.contact || !inviteForm.email) {
      toast.error("Please fill required fields", { description: "Company name, contact person, and email are required." });
      return;
    }
    ...
    const newPartner: MockPartner = {
      ...
      logoInitials: inviteForm.company.split(" ").slice(0, 2).map(w => w[0]!).filter(Boolean).join("").toUpperCase() || "XX",
    ...
    setInviteOpen(false);
    setInviteForm({ company: "", contact: "", email: "", phone: "", category: "Auto", manager: "Aisha Olamide" });
  };
  ...
  return (
    <AdminShell activeNav="partners" title="Partners">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <Button variant="primary" onClick={() => setInviteOpen(true)} className="rounded-full h-11 px-5">
      ...
      <section className="mb-5 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {kpis.map((s, i) => {
          const bg: Record<string, string> = { ink: "bg-ink/8", mint: "bg-mint/25", lemon: "bg-lemon/35", coral: "bg-coral/15" };
          const ic: Record<string, string> = { ink: "text-ink", mint: "text-ink", lemon: "text-ink", coral: "text-coral" };
          return (
            <Card key={i} className="rounded-[24px] border-0 bg-white p-0 ring-1 ring-ink/8 shadow-sm">
              <CardContent className="flex items-center gap-4 p-5">
                <div className={cn("grid size-12 place-items-center rounded-2xl shrink-0", bg[s.tone])}>
                  <s.icon className={cn("size-5", ic[s.tone])} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-ink/45 whitespace-nowrap">{s.label}</p>
                  <p className="font-display text-xl font-extrabold text-ink mt-0.5 whitespace-nowrap">{s.val}</p>
                </div>
              </CardContent>
            </Card>
          );
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select defaultValue="all">
              ...
              <SelectContent className="rounded-[22px] bg-white p-1.5 ring-1 ring-ink/10">
                {CATEGORIES.map((cat) => (
                  <SelectItem key={cat} value={cat === "All categories" ? "all" : cat.toLowerCase().replace(/[^a-z]/g, "")} className="rounded-xl font-bold">
                    {cat}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <Table>
              <TableHeader className="[&_tr]:border-ink/10">
                <TableRow>
                  ...
                  <TableHead className="py-3.5 font-extrabold text-ink/65">Contact person</TableHead>
                  ...
                  <TableHead className="py-3.5 text-right font-extrabold text-ink/65">Assets listed</TableHead>
                  <TableHead className="py-3.5 text-right font-extrabold text-ink/65">Revenue driven</TableHead>
                  <TableHead className="py-3.5 font-extrabold text-ink/65">Status</TableHead>
                  <TableHead className="py-3.5 text-right font-extrabold text-ink/65">Actions</TableHead>
                </TableRow>
              </TableHeader>
                  ...
                  <TableRow key={p.id} className="hover:bg-lilac/10">
                    ...
                    <TableCell className="py-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-ink/65 truncate max-w-[200px]">{p.email}</span>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="py-3.5">
                      <Badge className={cn("rounded-full px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wider ring-0", statusPill[p.status])}>
                        {p.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {p.status === "PENDING" && (
                          <>
                            <Button
                              ...
                              onClick={() => {
                                const next = partners.map(x => x.id === p.id ? { ...x, status: "APPROVED" as PartnerStatus, approved: true } : x);
                                setPartners(next); partnerList = next;
                                toast.success("Partner approved", { description: `${p.name} is now active.` });
                              }}
                            >
                            ...
                            <Button
                              ...
                              onClick={() => {
                                const next = partners.map(x => x.id === p.id ? { ...x, status: "REJECTED" as PartnerStatus, approved: false } : x);
                                setPartners(next); partnerList = next;
                                toast.info("Partner rejected", { description: `${p.name} · rejection sent.` });
                              }}
                            >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 rounded-[22px] bg-white p-1.5 ring-1 ring-ink/10">
                            ...
                            {p.status === "APPROVED" && (
                              <DropdownMenuItem
                                ...
                                onClick={() => {
                                  const next = partners.map(x => x.id === p.id ? { ...x, status: "SUSPENDED" as PartnerStatus, approved: false } : x);
                                  setPartners(next); partnerList = next;
      ...
      <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
        <DialogContent className="rounded-[28px] bg-white border-0 ring-1 ring-ink/10 shadow-xl max-w-[520px] sm:max-w-[560px] p-0 overflow-hidden">
          <div className="bg-gradient-to-r from-mint/25 via-sky/15 to-lemon/30 px-7 pt-7 pb-5">
            <DialogHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DialogTitle className="font-display text-2xl font-extrabold text-ink">Invite a partner</DialogTitle>
                  <DialogDescription className="text-sm font-bold text-ink/55 mt-1">
                    Send an onboarding invite to a prize asset partner. They'll receive a setup email.
                  </DialogDescription>
                </div>
              </div>
            </DialogHeader>
          ...
          <div className="space-y-4 px-7 py-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2 space-y-2">
                <Label className="text-[10px] font-extrabold uppercase tracking-wider text-ink/45">Company name *</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-extrabold uppercase tracking-wider text-ink/45">Contact person *</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-extrabold uppercase tracking-wider text-ink/45">Email *</Label>
                <Input
                  type="email"
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-extrabold uppercase tracking-wider text-ink/45">Phone</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-extrabold uppercase tracking-wider text-ink/45">Partner category</Label>
                <Select value={inviteForm.category} onValueChange={(v) => setInviteForm(f => ({ ...f, category: v }))}>
              ...
              <div className="sm:col-span-2 space-y-2">
                <Label className="text-[10px] font-extrabold uppercase tracking-wider text-ink/45">Assigned manager</Label>
                <Select value={inviteForm.manager} onValueChange={(v) => setInviteForm(f => ({ ...f, manager: v }))}>
                  <SelectTrigger className="h-12 rounded-2xl bg-white ring-1 ring-ink/10 px-4 text-sm font-extrabold text-ink shadow-none focus:ring-coral">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="rounded-[22px] bg-white p-1.5 ring-1 ring-ink/10">
          ...
          <DialogFooter className="px-7 pb-7 pt-0 flex-row gap-2 sm:justify-end">
            <Button variant="outline" onClick={() => setInviteOpen(false)} className="rounded-full h-11 px-6">
              Cancel
            </Button>
