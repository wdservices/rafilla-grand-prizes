...
const WEEKS = ["Wk27", "Wk28", "Wk29", "Wk30", "Wk31", "Wk32", "Wk33", "Wk34", "Wk35", "Wk36", "Wk37", "Wk38"];
        ...
        {WEEKLY_ENTRIES.map((v, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
              <span className={`text-[10px] font-mono font-bold ${isMax ? "text-coral" : "text-ink/50"} opacity-0 group-hover:opacity-100 transition`}>
                {v.toLocaleString()}
              </span>
              <div
...
const SETTLEMENTS: Settlement[] = [
  { id: "STL-W38-4021", period: "Week 38 · Sep 16 – 22", amountKobo: 182400000, status: "pending", date: "Pending · Sep 27" },
  { id: "STL-W37-3988", period: "Week 37 · Sep 09 – 15", amountKobo: 213600000, status: "processing", date: "Processing · Sep 23" },
  { id: "STL-W36-3944", period: "Week 36 · Sep 02 – 08", amountKobo: 145900000, status: "paid", date: "Paid · Sep 13" },
  { id: "STL-W35-3899", period: "Week 35 · Aug 26 – Sep 01", amountKobo: 98400000, status: "paid", date: "Paid · Sep 06" },
  { id: "STL-W34-3855", period: "Week 34 · Aug 19 – 25", amountKobo: 128700000, status: "paid", date: "Paid · Aug 30" },
];

const TOP_FIRST = ["Tunde", "Amaka", "Chidi", "Sade", "Funmi", "Kemi", "Bola", "Ifeoma", "Dele", "Zainab", "Emeka", "Ngozi"];
const TOP_LAST = ["Bakare", "Okafor", "Eze", "Lawal", "Adeyemi", "Hassan", "Tinubu", "Dike", "Ogun", "Aliyu", "Nwosu", "Obi"];
const TINTS = ["coral", "mint", "lemon", "sky", "lilac"];

function statusBadge(s: Settlement["status"]) {
  if (s === "paid")
    return <Badge className="rounded-full bg-mint/40 border-mint text-ink text-[10px] font-bold">PAID</Badge>;
  if (s === "processing")
    return <Badge variant="outline" className="rounded-full bg-sky/20 border-sky text-ink text-[10px]">PROCESSING</Badge>;
  return <Badge variant="outline" className="rounded-full bg-lemon/30 border-lemon text-ink text-[10px]">PENDING</Badge>;
}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="font-body text-ink/60 text-sm mt-1">
              Here's how Lekki Luxury Autos is performing on Rafilla this week.
            </p>
          </div>
            ...
            <Card key={k.label} className="border-ink/10 overflow-hidden">
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-4">
                  <div className={`p-2.5 rounded-2xl border ${k.tint}`}>
                    {k.icon}
                  </div>
                  <Badge variant="outline" className="rounded-full text-[10px] bg-cream text-ink/60 border-ink/10 font-body">
                    <TrendingUp className="w-3 h-3 mr-1 text-mint" /> +{10 + (k.label.length % 20)}%
                  </Badge>
                </div>
                <p className="text-xs font-body uppercase tracking-wider text-ink/50 mb-1">{k.label}</p>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline" className="rounded-full bg-coral/10 border-coral/30 text-coral">
            ...
            <CardContent className="pt-0">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 rounded-xl bg-cream/60">
                  <p className="text-[10px] uppercase tracking-wider text-ink/50 font-body">12w total</p>
                  <p className="font-display text-xl text-ink mt-0.5">5,149</p>
                </div>
                <div className="p-3 rounded-xl bg-cream/60">
                  <p className="text-[10px] uppercase tracking-wider text-ink/50 font-body">Avg / listing</p>
                  <p className="font-display text-xl text-ink mt-0.5">1,562</p>
                </div>
                <div className="p-3 rounded-xl bg-coral/10 border border-coral/20">
                  <p className="text-[10px] uppercase tracking-wider text-coral font-body">Best listing</p>
          ...
          <Card className="border-ink/10">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="font-body text-sm">Players with most entries</CardDescription>
            </CardHeader>
                ...
                return (
                  <div key={i} className="flex items-center gap-3">
                    <Badge variant="outline" className={`rounded-full w-7 h-7 p-0 justify-center shrink-0 ${
                      i === 0 ? "bg-coral text-white border-coral" : i === 1 ? "bg-lemon/40 border-lemon" : "bg-cream border-ink/20"
                    }`}>
                      {i + 1}
                    </Badge>
                    <Avatar className="w-9 h-9 shrink-0">
                      <AvatarFallback className={`bg-${TINTS[i % TINTS.length]} text-ink font-display font-semibold text-sm`}>
                        {first[0]!}{last[0]!}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <p className="font-body text-sm text-ink font-semibold truncate">{first} {last}</p>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <CardDescription className="font-body text-sm">Weekly disbursements to your Zenith account</CardDescription>
                </div>
              ...
              {SETTLEMENTS.map((s) => (
                <div key={s.id} className="py-3 flex items-center gap-3 flex-wrap">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] font-mono text-ink/40">{s.id} · {s.date}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-display text-lg text-ink font-bold">{formatNaira(s.amountKobo)}</p>
          ...
          <Card className="lg:col-span-2 bg-gradient-to-br from-ink via-ink to-ink/90 text-cream overflow-hidden relative">
            ...
            <CardContent className="p-6 relative">
              <div className="flex items-start justify-between mb-6">
                ...
                <Badge className="rounded-full bg-coral text-white border-coral font-bold">NEW</Badge>
              </div>
              ...
              <div className="space-y-2 mb-6">
                {[
                  { icon: <Car className="w-4 h-4" />, label: "Luxury vehicles · ₦20M+ avg ticket" },
                  { icon: <Watch className="w-4 h-4" />, label: "Watches & jewelry · High sell-through" },
                  { icon: <Smartphone className="w-4 h-4" />, label: "Electronics · Fastest draws" },
                  { icon: <Gem className="w-4 h-4" />, label: "Real estate · Premium carousel slot" },
                ].map((t) => (
                  <div key={t.label} className="flex items-center gap-2 text-cream/80 text-sm font-body">
              ...
              <p className="text-center text-[11px] text-cream/50 mt-3 font-body">
                <Calendar className="w-3 h-3 inline mr-1" /> Typical review · 24 hours · 94% approval rate
              </p>
            </CardContent>
          </Card>
        </div>
