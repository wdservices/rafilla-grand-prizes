...
export function RegisterForm() {
  ...
  async function handleUsernameBlur() {
    ...
    if (!format.valid) {
      setErrors((prev) => ({ ...prev, username: format.error || "Invalid username" }));
  ...
  async function handleEmailBlur() {
    ...
    if (!format.valid) {
      setErrors((prev) => ({ ...prev, email: format.error || "Invalid email" }));
    ...
    try {
      ...
      if (!res.available) {
        setErrors((prev) => ({ ...prev, email: res.error || "This email is already registered" }));
        setEmailAvailable(false);
      } else {
        setErrors((prev) => {
          const n = { ...prev };
  ...
  function validateStep1Sync(): { valid: boolean; nextErrors: Record<string, string> } {
    const next: Record<string, string> = {};
    const uFormat = validateUsernameFormat(username);
    if (!uFormat.valid) next.username = uFormat.error || "Invalid username";

    const eFormat = validateEmailFormat(email);
    if (!eFormat.valid) next.email = eFormat.error || "Invalid email";
    ...
    if (!password || password.length < 8) {
      ...
      next.password = "Add uppercase or symbols for a stronger password";
    }
    if (password !== confirmPassword) {
      next.confirmPassword = "Passwords do not match";
    }
    return { valid: Object.keys(next).length === 0, nextErrors: next };
  }

  function validateStep2() {
    const next: Record<string, string> = {};
    if (!address || address.length < 10)
      next.address = "Please enter a complete residential address";
    if (!dob || !isAdult(dob)) next.dob = "You must be 18 years or older";
    if (!terms) next.terms = "You must agree to the Terms & Privacy Policy to create an account";
    setErrors(next);
    return Object.keys(next).length === 0;
  }

  async function onNext() {
    const syncResult = validateStep1Sync();
    if (!syncResult.valid) {
      setErrors(syncResult.nextErrors);
      return;
    }
    ...
    try {
      ...
      if (!uRes.available) {
        asyncErrors.username =
          uRes.error || "This username is already taken. Please choose another.";
        setUsernameAvailable(false);
      } else {
        setUsernameAvailable(true);
      }
      ...
      if (!eRes.available) {
        asyncErrors.email =
          eRes.error || "This email is already registered. Please sign in instead.";
        setEmailAvailable(false);
      } else {
        setEmailAvailable(true);
      }
      ...
      setCheckingStep1(false);
      setErrors({ global: err?.message || "Failed to verify account details. Please try again." });
    }
  }
  ...
  return (
    <form noValidate onSubmit={onSubmit} className="space-y-5">
      ...
      {step === 1 ? (
        <div className="space-y-4">
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label htmlFor="r-username" className="text-xs font-extrabold text-ink/60">
                Username (unique)
              </label>
              {checkingUsername ? (
                <span className="flex items-center gap-1 text-[11px] font-bold text-ink/40">
                  <Loader2 className="size-3 animate-spin" /> Checking...
                </span>
              ) : usernameAvailable === true && !errors.username ? (
                <span className="flex items-center gap-1 text-[11px] font-extrabold text-mint">
            ...
            <div className="relative">
              <input
                ...
                placeholder="e.g. adebayotunmise"
                value={username}
                onChange={(e) => {
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label htmlFor="r-email" className="text-xs font-extrabold text-ink/60">
            ...
            <div className="relative">
              <input
                ...
                className={cn(
                  authInputBase(errors.email),
                  "w-full",
                  emailAvailable === true && !errors.email && "border-mint/50 bg-mint/5",
                )}
              />
            </div>
          ...
          <div className="space-y-1.5">
            <label htmlFor="r-phone" className="text-xs font-extrabold text-ink/60">
              Phone number
            </label>
            <input
              id="r-phone"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label htmlFor="r-password" className="text-xs font-extrabold text-ink/60">
                Password
              </label>
              <div className="relative">
                <input
            ...
            <div className="space-y-1.5">
              <label htmlFor="r-confirm" className="text-xs font-extrabold text-ink/60">
                Confirm
              </label>
              <div className="relative">
                <input
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label htmlFor="r-address" className="text-xs font-extrabold text-ink/60">
              Residential address
            </label>
            <input
              id="r-address"
          ...
          <div className="space-y-1.5">
            <label htmlFor="r-dob" className="text-xs font-extrabold text-ink/60">
              Date of birth
            </label>
            <input
              id="r-dob"
            ...
            />
            <p className="px-1 text-[11px] font-extrabold text-ink/40">
              Must be 18 or older to participate
            </p>
            <FieldError error={errors.dob} />
          </div>

          {/* Terms & Privacy checkbox under Profile before creating account */}
          <div className="space-y-1.5 pt-1">
            ...
            >
              ...
              <label
                htmlFor="r-terms"
                className="text-[12px] leading-relaxed font-bold text-ink/65 cursor-pointer select-none"
              >
                I agree to the{" "}
                <Link
      ...
      {step === 1 ? (
        ...
        >
          {checkingStep1 ? <Loader2 className="size-4 animate-spin" /> : null}
          {checkingStep1 ? "Verifying availability..." : "Continue"}
        </Button>
      ) : (
        <div className="flex gap-3">
          <Button
            ...
            variant="outline"
            size="lg"
            className="flex-1 h-12 rounded-xl border-ink/15"
            onClick={() => setStep(1)}
          >
            Back
          </Button>
